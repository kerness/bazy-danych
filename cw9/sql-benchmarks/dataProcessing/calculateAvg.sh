...
# MYSQL
_1ZL_MYSQL_normal=$(awk -v N=1 '{ sum += $N } END { if (NR > 0) print sum / NR }' res/1ZL_MSQL_normal.txt)
_2ZL_MYSQL_normal=$(awk -v N=1 '{ sum += $N } END { if (NR > 0) print sum / NR }' res/2ZL_MSQL_normal.txt)
_3ZG_MYSQL_normal=$(awk -v N=1 '{ sum += $N } END { if (NR > 0) print sum / NR }' res/3ZG_MSQL_normal.txt) 
_4ZG_MYSQL_normal=$(awk -v N=1 '{ sum += $N } END { if (NR > 0) print sum / NR }' res/4ZG_MSQL_normal.txt)

_1ZL_MYSQL_idx=$(awk -v N=1 '{ sum += $N } END { if (NR > 0) print sum / NR }' res/1ZL_MSQL_idx.txt) 
_2ZL_MYSQL_idx=$(awk -v N=1 '{ sum += $N } END { if (NR > 0) print sum / NR }' res/2ZL_MSQL_idx.txt) 
_3ZG_MYSQL_idx=$(awk -v N=1 '{ sum += $N } END { if (NR > 0) print sum / NR }' res/3ZG_MSQL_idx.txt)
_4ZG_MYSQL_idx=$(awk -v N=1 '{ sum += $N } END { if (NR > 0) print sum / NR }' res/4ZG_MSQL_idx.txt) 
